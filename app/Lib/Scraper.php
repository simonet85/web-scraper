 ...
 * Class Scraper
 * 
 * Handles and processes scraping using specific links
 * First we work on the main filter expression which is the container of the items,
 * then using annonymous callback on the filter function we iterate
 * and save results into the article table.
 * 
 * @package  App\Lib
 */
class Scraper
{
                        ...
                        if ($node->filter($val['selector'])->count() > 0) {
                            if ($val['is_attribute'] == false) {
                                ...
                                if ($key == 'source_link') {
                                    ...
                                    // append website url in case the article is note full url
     ...
     * fetchFullContent
     * 
     * this method pulls the full content of a single item using the item url and selector
     * 
     * @param  $item_url
     * @param  $selector
     * @return string
